...
public class RemoveFavoriteFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_remove_favorite, container, false);

        UrlBean getUrl = new UrlBean();
        url = getUrl.getRemoveFavorites();
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            if (response.getInt(KEY_STATUS) == 0) {
                                ...
                                FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                                FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                                fragmentTransaction.replace(R.id.fragment_container, new SuccessRemoveFavoritesFragment());
                                fragmentTransaction.addToBackStack(null);
                                fragmentTransaction.commit();
